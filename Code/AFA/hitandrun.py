...
class HitAndRun(object):
    ...
    # private functions
    def _step(self, max_repeat=1000):
        """Make one step."""
        i = 0
        while i < max_repeat:
            ...
            if len(self.lambdas[self.lambdas > 0]) >0 and len(self.lambdas[self.lambdas < 0]) > 0:
                ...
                return
            i+=1
        self.current = self.starting_point #go back to the start
