...
def iid_gf(label_budget, model = h_star):
    Sx1 = np.array([positive_random_input_generator(X) for _ in range(label_budget + 1)])
    Sx0 = np.array([negative_random_input_generator(X) for _ in range(label_budget + 1)])
...
with open('student_RF_Experiments.pkl', 'wb') as handle:
